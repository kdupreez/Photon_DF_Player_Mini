 ...
 **Wire: **Board : Photon
		*Photon RX - player TX; 
		*Photon TX - player RX;
		*Photon D3  - player BUSY

 *	Note: the mp3 files must put into mp3 folder in your tf card 
 */


#include "Photon_DF_Player_Mini/Photon_DF_Player_Mini.h"

void setup () 
{
    Serial.begin (9600); //Serial comms to PC

    Serial1.begin (9600); // Serial1 pins connected to MP3 board
...
void loop () 
{   
    boolean play_state = digitalRead(D3);
    if(play_state == HIGH)
    {
        mp3_next ();
    }
